...
def apply_coord_to_df(func, df, in_cols, out_cols):
    """ applying one of the above functions to a dataframe."""

    df['coord_tuple'] = df.apply(lambda x: func(x[[in_cols[0]]].values, x[[in_cols[1]]].values), axis=1)
    df[out_cols] = pd.DataFrame(df['coord_tuple'].tolist(), index=df.index)
    df = df.drop('coord_tuple', axis=1)
    return df